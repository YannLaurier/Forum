...
$thatGuy = User::bringOneUser($bdd, $_GET["pseudo"]);

$thatGuysPosts = Post::bringThatGuysPosts($bdd, $thatGuy[0]["id"]);
$thatGuysAns = Answer::bringThatGuysAns($bdd, $thatGuy[0]["id"]);
        ...
        <div class="container">
            <section id="user_info" class="flex between">
                ...
                if (!empty($thatGuy[0]["profilePicData"])) {
                    echo "actions\displayProfilePic.php?id=" . $_GET["pseudo"];
                } else {
                    echo "assets/default.jpg";
                }
                ?> alt=<?php echo "profile picture of" . $thatGuy[0]["Pseudo"]; ?>>
                <?php if ($_SESSION["user"]["Pseudo"] === $_GET["pseudo"]) {
                    echo "<button type='button' id='editPp' class='tinyGuy' data-toggle='modal' popovertarget='editProfilePic'>";
                    if (!empty($thatGuy[0]["profilePicData"])) {
                        echo '<svg fill="var(--light)" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
	                    width="20px" height="20px" viewBox="0 0 568.017 568.017"
	                    xml:space="preserve">
                        ...
                        echo '
                        <svg fill="var(--light)" width="20px" height="20px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns">
                    ...
                    echo '</button>
                          <dialog id="editProfilePic" class="pad-10 popover" popover>
                            <form method="POST"  action="actions/editProfilePic.php" class="flex column bigInput" enctype="multipart/form-data">
                                <div class="flex between">
                                    <input type="file" name="newPp" id="newPp">
                                    <button type="button" class="close">
                                        <svg fill="var(--dark)" xmlns="http://www.w3.org/2000/svg" width="17.828" height="17.828">
                            ...
                            <p class="thisEmptyMessage">Assure-toi que ton fichier est plus petit que 2Go, autrement il ne sera pas uploadé !</p>
                          </dialog>';
                }

                ?>
                <div class="flex column">
                    <?php if($_SESSION["user"]["Pseudo"] === $thatGuy[0]["Pseudo"]){ ?>
                    ...
                    <dialog class="pad-10 popover" id="editProfileForm" aria-modal="true" popover>
                        <form method="POST" action="actions/editProfile.php" class="flex column">
                            ...
                            <textarea name="newDesc" id="newDesc"
                                value="<?php echo $thatGuy[0]["description"]; ?>"
                                placeholder="Vous savez, moi je ne crois pas qu'il y ait de bonne ou de mauvaise situation. Moi, si je devais résumer ma vie aujourd'hui avec vous, je dirais que c'est d'abord des rencontres. Des gens qui m'ont tendu la main, peut-être..."></textarea>
                            <div>
                                <p>Montre-nous que c'est bien toi en entrant ton mot de passe actuel :</p>
                                <input name="oldPword" type="password">
                            </div>
                            <button type="submit" name="editProfile" value="<?php echo $_SESSION["user"]["id"]; ?>">Confirmer les changements</button>
                        </form>
                    </dialog>
                    <?php } ?>
                    <div class="text_right">
                        <h2><?php echo $thatGuy[0]["Pseudo"]; ?></h2>
                        <p><?php echo $thatGuy[0]["email"]; ?></p>
                    </div>
                </div>
                <div>
                    <p>
                        <?php
                        if (!empty($thatGuy[0]["description"])) {
                            echo $thatGuy[0]["description"];
                        } else {
            ...
            <section id="recent_posts">
                ...
                <?php
                    foreach($thatGuysPosts as $tab){
                        ?>
                        <div class="list pad-10"><a href="posts.php?id=<?php echo $tab["id"]; ?>"><?php echo $tab["title"]; ?></a></div>
                        <?php
                    }
                ?>
            </section>
            <section id="recent_answers">
                ...
                <?php
                    foreach($thatGuysAns as $tab){
                        ?>
                        <div class="list pad-10"><a href="posts.php?id=<?php echo $tab["FK_post_id"]; ?>"><?php echo substr($tab["content"], 0, 150); ?></a></div>
                        <?php
                    }
                ?>
            </section>
